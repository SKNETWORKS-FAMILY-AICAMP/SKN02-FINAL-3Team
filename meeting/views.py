...
def detail_view(request, meeting_id):
    meeting = get_object_or_404(Meeting, pk=meeting_id)
    Participants = Participant.objects.filter(meeting_id=meeting_id)
    Checkers_id = Participant.objects.filter(is_checker=True, meeting_id=meeting_id)
    Users = User.objects.filter(id__in=Participants.values_list('user_id', flat=True))
    CheckerUsers = User.objects.filter(id__in=Participants.values_list('user_id', flat=True))
    return render(request, 'meeting_detail.html', {'meeting': meeting, 'Participants': Participants, 'Users': Users, 'CheckerUsers': CheckerUsers})
